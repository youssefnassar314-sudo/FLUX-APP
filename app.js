...
function buildReceiptSections() {
    ...
    let foodRows = foodByDay.map(day => { let dayKey = Object.keys(aiByDay).length > 0 ? new Date(day.items[0].createdAt).toLocaleDateString('en-CA') : null; let dayVerdict = dayKey && aiByDay[dayKey] ? aiByDay[dayKey] : null; return `<div class="receipt-day-header">${day.label}</div>${day.items.map(f => `<div class="receipt-row"><span class="r-label">• ${f.item}${f.source ? ` <span style="opacity:0.6;">· ${f.source}</span>` : ''}${f.notes ? ` <span style="opacity:0.6;">(${f.notes})</span>` : ''}</span><span class="r-val">${f.cost > 0 ? '₱' + f.cost.toFixed(2) : '—'}</span></div>`).join('')}${dayVerdict ? `<div class="receipt-ai-box">AI: ${dayVerdict}</div>` : ''}`; }).join('') || '<p style="text-align:center;font-size:10px;color:#888;letter-spacing:1px;margin:12px 0;">NO FOOD LOGGED</p>';
